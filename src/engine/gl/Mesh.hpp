...
protected:
    ...
    GLsizei count;                  // Number of elements to draw
...
public:
    ...
    static Mesh create_plane(glm::vec3 center = glm::vec3(0.0f), glm::vec2 size = glm::vec2(1.0f),
        glm::vec2 tiling = glm::vec2(1.0f));
    static Mesh create_sphere(glm::ivec2 segments = { 32, 16 }, glm::vec3 center = glm::vec3(0.0f),
        float radius = 1.0f);
    static Mesh create_cylinder(int segments = 32, glm::vec3 center = glm::vec3(0.0f),
        float height = 1.0f, float radius = 1.0f);

    // ============== SETTERS ==============
    void set_vertices(const std::span<Vertex>& vertices);
    ...
    uint16_t get_index(size_t index) const;
